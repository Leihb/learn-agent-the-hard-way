// Learn Agent the Hard Way — 练习 17：按需触发
//
// 练习 16 证明了"发现 + 注入"能跑通：清单进系统提示，正文靠 skill 工具
// 按需加载。这一章问两个更尖锐的问题：模型触发的依据是清单里那句
// description，不是正文——它会不会真的先去加载正文确认一遍，还是凭一句
// 话就动手？以及，这一套"清单便宜、正文贵"到底贵在哪、便宜在哪，用真实
// token 数字说话，不能只停留在断言。
package main
...
func main() {
	...
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, `用法: ./ex17 "你的任务"  或  ./ex17 -c <session-id> "你的任务"`)
		os.Exit(1)
	}
	task := args[0]
