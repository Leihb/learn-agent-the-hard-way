...
// 路子）。JavaScript 的工具从来没有 ctx 参数，沿用同一个简化：模块级
// 全局 CURRENT_BG，在 runInterruptible 开始跑一轮之前挂上、跑完摘下；
// 子 agent 的 runChildLoop 进门前把它设成 null（对应 Go 版
// `ctx = withBg(ctx, nil)`）——子 agent 的完成通知回不去一个只活一次
// 调用的循环，所以后台这条路对子 agent 整个关死，bash 静默塌回同步执行。
//
// 完成通知不需要像 Python 版那样另起一个转发线程桥接进 EVENTS——单线程
// 下 BgManager 的收尾回调直接 `EVENTS.push({tag:"bg", note})` 就够了，
// 跟 Waker.fire() 直接推事件是同一个道理。
