// Learn Agent the Hard Way — 练习 22：MCP 客户端——接入别人的工具
//
// 到上一章为止，注册表里的每一个工具都是我们自己写的代码。这一章打开
// 另一扇门：读 mcp.json 里配置的外部服务器，把它们各自声明的工具接进
// 同一张注册表。协议是 MCP——子进程 + 标准输入输出上的 JSON-RPC，一行
// 一个 JSON 对象。注册表这层一个字不用改：外部工具进来之后，也是"一份
// 给模型看的声明 + 一个真正干活的函数"，跟 read_file 没有区别。
