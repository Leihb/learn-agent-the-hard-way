...
//
// 到上一章为止，注册表里的每一个工具都是我们自己写的 Go 代码。这一章
// 打开另一扇门：读 mcp.json 里配置的外部服务器，把它们各自声明的工具
// 接进同一张注册表。协议是 MCP——子进程 + 标准输入输出上的 JSON-RPC，
// 一行一个 JSON 对象。注册表这层一个字不用改：外部工具进来之后，也是
// "一份给模型看的声明 + 一个真正干活的函数"，跟 read_file 没有区别。
package main
...
func main() {
	...
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, `用法: ./ex22 "你的任务"  或  ./ex22 -c <session-id> "你的任务"`)
		os.Exit(1)
	}
	task := args[0]
