...
# 同一个套路。这也是全书第一个真正的第三方依赖：websockets——Python
# 标准库没有 WebSocket 客户端，跟 Go 版破例引入 gorilla/websocket 是
# 同一个理由（传输层细节跟"agent 怎么用工具"这条主线不沾边）。这里用
# 的是 websockets 16 自带的同步客户端（`websockets.sync.client`），跟
# 本书从 ex20 起就定的线程并发风格对得上，不用改叫 asyncio。
#
# 语言分叉点：Go 版的 cdpClient 用一个 goroutine（readLoop）+ channel 分发
# 响应；Python 版沿用同一个并发风格——一个后台线程读 websocket，一个
# 字典 + 锁做"按 id 找回等待者"的分发表，跟 Go 的 pending map 是同一个
# 结构，只是 channel 换成了 queue.Queue(maxsize=1) 的一个槽位。
#
...
# gorilla/websocket 的默认 Dialer 不发 Origin 头，天然绕过这一关；
# websockets 的同步客户端同样默认不发 Origin 头（跟 Go 版行为一致，
# 实测过），不需要额外处理——这条坑只在另一个 Python 库
# `websocket-client` 上踩到过（它的 create_connection 会主动补一个
# Origin 头模拟浏览器，反而触发这道新校验），换成 websockets 库之后
# 这条坑就不存在了，记在这里是为了交代清楚，不是说三语言都要踩一遍。
import base64
import html
import json
import os
...
# websockets——全书第一个第三方依赖，见文件顶部的说明。
...
class GrepTool:
    def definition(self):
        return {
            ...
            "description": ("用 ripgrep 搜索文件内容。pattern 是正则（Rust 正则语法）。"
                             ...
                             f"遵守 .gitignore。最多返回 {GREP_MAX_LINES} 行——"
    ...
    def execute(self, args):
        ...
        if mode in ("", "content"):
            ...
            rg_args.append("--count-matches")
        else:
            return f"错误: 不认识的 mode {mode!r}（可选 content | files_with_matches | count）"
        if in_.get("case_insensitive"):
            rg_args.append("-i")
        include = in_.get("include", "") or ""
        if include:
            rg_args += ["--glob", include]
        context_lines = in_.get("context_lines") or 0
        if context_lines > 0 and mode in ("", "content"):
            rg_args += ["-C", str(context_lines)]
        rg_args += ["--", pattern]
        path = in_.get("path", "") or ""
        if path:
            rg_args.append(path)
        ...
        text = proc.stdout.decode(errors="replace").rstrip("\n")
        if not text:
            return "(没有匹配)"
        lines = text.split("\n")
        if len(lines) > GREP_MAX_LINES:
            head = "\n".join(lines[:GREP_MAX_LINES])
            return (f"{head}\n\n[截断：只显示前 {GREP_MAX_LINES} 行，共 {len(lines)} 行——"
                     "收窄 pattern、加 include，或者指定更具体的 path]")
        return text
...
class GlobTool:
    def definition(self):
        return {
            ...
            "description": ("按 glob 模式找文件。支持 `**` 匹配任意层目录（如 `src/**/*.go`）。"
                             f"最多返回 {GLOB_MAX_RESULTS} 个路径，按修改时间倒序"
    ...
    def execute(self, args):
        ...
        try:
            ...
            return f"错误: 模式 {pattern!r} 不合法: {e}"

        root = in_.get("path", "") or WORK_DIR
        try:
            ...
            return "错误: rg --files 失败: " + str(e)
        if proc.returncode != 0:
            return "错误: rg --files 失败: " + proc.stderr.decode(errors="replace")

        matches = []  # [(path, mtime)]
        for rel in proc.stdout.decode(errors="replace").rstrip("\n").split("\n"):
            if not rel or not regexp.match(rel):
                continue
            mtime = 0.0
            try:
                mtime = os.stat(os.path.join(root, rel)).st_mtime
            except OSError:
                pass
...
# `?` 单字符，其余字符原样。标准库的 fnmatch 不区分 `*` 和 `**`，与其绕着
# 它的语义打补丁，不如十几行编译一个。
...
class WebFetchTool:
    ...
    def execute(self, args):
        ...
        try:
            ...
            head = e.read(512)
            return f"错误: HTTP {e.code}: {head.decode(errors='replace').strip()}"
        except OSError as e:
            return "错误: 请求失败: " + str(e)
        with resp:
            # 只收文本。二进制响应转成字符串是一堆乱码，白白烧上下文，不如
            # 一句明白话指个路。
            ctype = resp.headers.get("Content-Type", "")
            if not is_textual_content_type(ctype):
                return f"这个 URL 返回的是二进制内容（{ctype}），web_fetch 只处理文本。要下载它，用 bash 的 curl -o。"
            body = resp.read(WEB_FETCH_MAX_BYTES)
        text = body
        if clean and "html" in ctype.lower():
            text = strip_html_to_text(body.decode("utf-8", errors="replace")).encode("utf-8")
        return tail(text, WEB_FETCH_MAX_RETURN)


# is_textual_content_type 判断响应是不是文本。空 Content-Type 按文本处理
# ——宁可让模型看到一点乱码，也别把一个没写头的纯文本接口拒之门外。
def is_textual_content_type(ctype):
    if not ctype:
        return True
    ctype = ctype.lower()
    if ctype.startswith("text/"):
        return True
    return any(s in ctype for s in ("json", "xml", "javascript", "yaml", "x-www-form-urlencoded"))


_RE_SCRIPT_STYLE = re.compile(r"<(?:script|style|noscript)[^>]*>.*?</(?:script|style|noscript)>",
                              re.IGNORECASE | re.DOTALL)
_RE_TAG = re.compile(r"<[^>]*>", re.DOTALL)
_RE_BLANK_LINES = re.compile(r"\n{3,}")


# strip_html_to_text 把 HTML 粗剥成可读文本：去掉脚本和样式、去掉所有标签、
...
WEB_SEARCH_DEFAULT_MAX = 5
WEB_SEARCH_HARD_MAX = 20

# BRAVE_ENDPOINT / BING_ENDPOINT 不是常量而是模块变量——单元测试要把它们
# 指到本地假服务器上。octo 同款手法。
...
class WebSearchTool:
    ...
    def execute(self, args):
        ...
        try:
            ...
            return "错误: 参数不是合法 JSON: " + str(e)
        query = (in_.get("query") or "").strip()
        if not query:
            return "错误: query 不能为空"
        max_results = in_.get("max_results") or 0
        if max_results < 1:
            max_results = WEB_SEARCH_DEFAULT_MAX
        if max_results > WEB_SEARCH_HARD_MAX:
            max_results = WEB_SEARCH_HARD_MAX
...
# search_brave 调 Brave Search API：一个 GET，key 放请求头，响应是干净
# 的 JSON。付费后端的样子——没有解析别人网页的狼狈。
def search_brave(query, max_results):
    url = f"{BRAVE_ENDPOINT}?q={urllib.parse.quote(query)}&count={max_results}"
    ...
    out = []
    for r in body.get("web", {}).get("results", []):
...
_RE_BING_BLOCK = re.compile(r'<li class="b_algo"')
_RE_BING_LINK = re.compile(r'<h2[^>]*><a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', re.IGNORECASE | re.DOTALL)
_RE_BING_SNIP = re.compile(r"<p[^>]*>(.*?)</p>", re.IGNORECASE | re.DOTALL)


# search_bing 抓 Bing 的结果页。免费兜底的样子——发一个装成浏览器的请求，
# 在别人的 HTML 里翻自己要的东西。
def search_bing(query, max_results):
    url = f"{BING_ENDPOINT}?q={urllib.parse.quote(query)}"
    req = urllib.request.Request(url, headers={
        "User-Agent": BROWSER_USER_AGENT,
    ...
    try:
        ...
        raise OSError(f"HTTP {e.code}") from None
    with resp:
        body = resp.read(2 * 1024 * 1024).decode(errors="replace")

    blocks = _RE_BING_BLOCK.split(body)
    out = []
    for block in blocks[1:]:
        i = block.find("<li class=")
        if i >= 0:
            block = block[:i]
        link = _RE_BING_LINK.search(block)
        if not link:
            continue
        result = {"url": html.unescape(link.group(1)), "title": clean_inline_html(link.group(2)), "snippet": ""}
        snip = _RE_BING_SNIP.search(block)
        if snip:
            result["snippet"] = clean_inline_html(snip.group(1))
        out.append(result)
...
# clean_inline_html 清一段行内 HTML：去标签、解实体、并空白。
...
class CDPClient:
    ...
    # call 发出一个命令并等它的响应。timeout 秒等不到就当超时处理——
    # Python/JS 版的工具从来没有 ctx 参数（练习 9 起就没有），这里用一个
    # 简单的超时代替 Go 版 ctx.Done() 的即时打断语义。
    def call(self, session_id, method, params, timeout=10):
        ...
        try:
            ...
            raise RuntimeError(f"cdp 发送 {method}: {e}") from None
        try:
            resp = q.get(timeout=timeout)
        except queue.Empty:
            with self.pending_lock:
                self.pending.pop(msg_id, None)
            raise TimeoutError(f"cdp 调用 {method} 超时（{timeout}s）") from None
        if resp is _CLOSED:
            raise RuntimeError(f"cdp 连接已断: {self.close_err}")
        if resp.get("error"):
            err = resp["error"]
            raise RuntimeError(f"{method}: cdp 错误 {err.get('code')}: {err.get('message')}")
        return resp.get("result")


# chrome_profile_dirs 是各平台默认的 Chrome 用户数据目录——勾选框方式
...
def chrome_profile_dirs():
    ...
    return [os.path.join(home, ".config/google-chrome"), os.path.join(home, ".config/chromium")]


BROWSER_SETUP_GUIDE = """连不上 Chrome。两种连法任选：
...
# cdp_endpoint 找到浏览器级 CDP websocket 的地址。经典的
...
def connect_chrome():
    ...
    v = os.environ.get("CDP_PORT", "")
    if v.isdigit() and int(v) > 0:
        port = int(v)
    ws_url = cdp_endpoint(port)
    try:
        # open_timeout 是握手超时；max_size 撑大到 64MB——一张截图就能
        # 超过默认的 1MB 读取上限。websockets 的同步客户端默认不发
        # Origin 头，跟 Go 版 gorilla/websocket 的默认行为一致，不需要
        # 额外处理（见文件顶部关于这条坑的说明）。
...
# Page 是一个已附着的 tab：页面级命令都带着它的 session_id 走。
class Page:
    def __init__(self, cli, session_id, target_id):
        self.cli = cli
        self.session_id = session_id
        self.target_id = target_id


# new_tab 开一个新 tab 并附着上去。永远开自己的 tab，绝不劫持用户正开
...
def _page_eval(page, expr):
    # eval 在页面里执行一段 JS 表达式，返回它的值（不关心就不用接）。
    # 这是整个页面层的地基：下面的导航等待、找元素、observe，全是 eval
    # 的不同用法。
    ...
    if ex:
        ...
        if desc:
            ...
            msg = desc.split("\n", 1)[0]
        raise RuntimeError(f"页面里的 JS 抛了异常: {msg}")
...
def page_navigate(page, url):
    ...
    committed = time.monotonic() + 3
    while time.monotonic() < committed:
        try:
            href = _page_eval(page, "location.href")
        except RuntimeError:
            break
        if href != start:
            break
    ...
    while True:
        try:
            if _page_eval(page, "document.readyState") == "complete":
                return
        except RuntimeError:
            pass
        if time.monotonic() >= deadline:
            raise RuntimeError(f"导航到 {url}: 等页面加载完超时")
        time.sleep(0.1)


# element_center 找到选择器命中的第一个元素，滚进视野，返回中心点坐标。
...
def element_center(page, selector):
    expr = f"""(() => {{
      let el;
      try {{ el = document.querySelector({js_str(selector)}); }} catch (e) {{ return {{bad: true}}; }}
      if (!el) return null;
    ...
    res = _page_eval(page, expr)
    if res is None:
        raise RuntimeError(f"选择器 {selector!r} 没有匹配到元素——先用 observe 看看页面上有什么")
    if res.get("bad"):
        raise RuntimeError(f"选择器 {selector!r} 不是合法的 CSS——本工具只认纯 CSS 选择器，用 observe 拿现成的")
...
CLICK_MOVE_SETTLE = 0.06  # 60ms：鼠标移动到位和按下之间停的那一拍


# page_click 在元素中心补一次真实的鼠标手势：移动、按下、抬起，走 CDP 的
...
# page_type 聚焦到目标元素，把文本"输"进去。Input.insertText 相当于一次
...
# NAMED_KEYS 是几个常用键的 CDP 描述符：(key, code, virtualKeyCode)。
NAMED_KEYS = {
...
# page_press_key 发一次真实按键：keyDown + keyUp。type 的 insertText 不
...
OBSERVE_MAX = 60  # observe 一次最多列出的元素数

# _OBSERVE_JS 蒸馏自 octo 的 InteractiveDigest：扫可交互元素，每个生成
...
# page_observe 返回页面的文本摘要：URL、标题、可交互元素清单，每个元素
# 带一条能直接喂给 click/type 的 CSS 选择器。这是给没有眼睛的模型准备的
# "看"——页面在模型那里从来不是像素，是这份清单。
def page_observe(page):
    try:
        meta = _page_eval(page, "({url: location.href, title: document.title})") or {}
    except RuntimeError:
        meta = {}
    digest = _page_eval(page, _OBSERVE_JS) or {"items": [], "total": 0}
    lines = [f"页面: {meta.get('title', '')} — {meta.get('url', '')}", "", "可交互元素:"]
    items = digest.get("items", [])
    if not items:
        lines.append("(一个都没找到)")
    for it in items:
        text, selector = it.get("text", ""), it.get("selector", "")
        lines.append(f"- {text}  →  {selector}" if text else f"- {selector}")
...
SCREENSHOT_DIR = ".screenshots"  # 截图落盘的目录，和 .sessions/.trash 一样长在工作目录


# page_screenshot 把当前视口截成 PNG 存到本地，返回文件路径。图片本身
# 不进对话——本书两端的模型都没有视觉，一坨 base64 只会烧钱不会被
# "看见"。octo 在这里按模型能力分岔：有视觉的模型收到真正的图片内容块，
# 没有的只收到路径和一句"用 observe"。我们的版本只有后一半。
def page_screenshot(page):
    res = page.cli.call(page.session_id, "Page.captureScreenshot", {"format": "png"})
...
# THE_BROWSER 是全局唯一的浏览器会话：一条 CDP 连接 + 一个我们自己开的
# tab，所有 browser 调用共享。全局不是偷懒，是刻意的：navigate 完再
# click，模型期望的是"同一个页面"；而且勾选框那条路每次新拨号都要用户
# 在 Chrome 里点一次授权，连接能复用多久就该复用多久。
THE_BROWSER = {"lock": threading.Lock(), "cli": None, "page": None}


# browser_page 拿到当前可用的页面，没有就建。三层探活，一层比一层贵：
# tab 还活着直接用；tab 死了（用户随手关了）在原连接上开个新的——不用
# 重新拨号，也就不会再弹授权；连接也死了（Chrome 整个重启了）才重连。
def browser_page():
    with THE_BROWSER["lock"]:
        page = THE_BROWSER["page"]
        if page is not None:
            try:
                _page_eval(page, "1")
                return page
            except (RuntimeError, TimeoutError):
                THE_BROWSER["page"] = None
        cli = THE_BROWSER["cli"]
        if cli is not None:
            try:
                page = new_tab(cli)
                THE_BROWSER["page"] = page
                return page
            except (RuntimeError, TimeoutError):
                cli._shutdown(RuntimeError("连接探活失败"))
                THE_BROWSER["cli"] = None
        cli = connect_chrome()
        try:
            page = new_tab(cli)
        except (RuntimeError, TimeoutError):
            cli._shutdown(RuntimeError("开 tab 失败"))
            raise
        THE_BROWSER["cli"], THE_BROWSER["page"] = cli, page
        print("[浏览器已连接，开了自己的 tab——不动你已经打开的任何页面]", file=sys.stderr)
        return page


# close_browser_tab 关掉我们自己开的那个 tab。只关 tab，不动 Chrome——
# 那是用户的进程，不是我们启动的。退出时把借的东西还回去。
def close_browser_tab():
    with THE_BROWSER["lock"]:
        cli, page = THE_BROWSER["cli"], THE_BROWSER["page"]
        if cli is None or page is None:
            return
        try:
            cli.call("", "Target.closeTarget", {"targetId": page.target_id}, timeout=2)
        except (RuntimeError, TimeoutError):
            pass
        THE_BROWSER["page"] = None


BROWSER_EVAL_MAX_RETURN = 8 * 1024  # eval 结果交给模型的上限


# head_of 截断到前 max 字节，对齐到整行。tail 留结尾是给 bash 日志用的
# （新信息在末尾）；页面正文相反，开头才是正题。
def head_of(s, max_bytes):
    if len(s) <= max_bytes:
        return s
    cut = s[:max_bytes]
    i = cut.rfind("\n")
    if i > 0:
        cut = cut[:i]
    return f"{cut}\n[... 后面 {len(s) - len(cut)} 字节被截断 ...]"


BROWSER_ACTION_TIMEOUT = 45  # 单个动作的上限。页面卡住时一次 CDP 调用可能永远等不到回音
...
class BrowserTool:
    ...
    def execute(self, args):
        ...
        try:
            ...
            return "错误: " + str(e)
        # 整个动作套一个总的时间上限：页面卡住时一次 CDP 调用可能永远
        # 等不到回音，单个 cli.call() 的超时兜不住"好几次调用叠加起来
        # 拖过头"的情况。Go 版用 context.WithTimeout 包住整个 execute；
        # Python 的工具没有 ctx 参数可以传递取消信号（练习 9 起就没有），
        # 借一个 daemon 线程 + 队列顶替：45 秒还没返回就当超时处理，
        # 工具报错、这一轮继续——卡住的那个线程会自生自灭（Python 没法
        # 从外部强行打断一个线程，daemon=True 至少保证它不会拖住进程
        # 退出），不是完美的打断，但足够让轮次不被拖死。
        result_q = queue.Queue(maxsize=1)

        def worker():
            try:
                result_q.put(("ok", self._dispatch(page, action, in_)))
            except Exception as e:  # noqa: BLE001 —— 把任何意外也带回主线程
                result_q.put(("err", str(e)))

        threading.Thread(target=worker, daemon=True).start()
        try:
            kind, value = result_q.get(timeout=BROWSER_ACTION_TIMEOUT)
        except queue.Empty:
            return f"错误: 动作 {action} 超过 {BROWSER_ACTION_TIMEOUT} 秒没有响应，页面可能卡住了"
        if kind == "err":
            return "错误: " + value
        return value

    def _dispatch(self, page, action, in_):
        url = in_.get("url", "") or ""
        try:
            if action == "navigate":
                if not url:
                    return "错误: navigate 需要 url"
                page_navigate(page, url)
                return "已打开 " + url
            if action == "observe":
                return page_observe(page)
            if action == "click":
                selector = in_.get("selector", "") or ""
                if not selector:
            ...
            if action == "type":
                ...
                text = in_.get("text", "") or ""
                page_type(page, selector, text)
                return f"已在 {selector} 输入 {text!r}"
            if action == "key":
                keys = in_.get("keys", "") or ""
                if not keys:
                    return "错误: key 需要 keys"
                page_press_key(page, keys)
                return "已按下 " + keys
            if action == "eval":
                js = in_.get("js", "") or ""
                if not js:
                ...
                if out is None:
                    return "(没有返回值)"
                return head_of(json.dumps(out, ensure_ascii=False), BROWSER_EVAL_MAX_RETURN)
            if action == "screenshot":
                path = page_screenshot(page)
                return f"截图已存到 {path}。当前模型看不了图片，读页面请用 observe 或 eval"
