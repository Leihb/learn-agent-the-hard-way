// Learn Agent the Hard Way — 练习 21：编排交给代码还是模型
//
// 上一章的并发扇出有一个前提：模型得在同一轮里把几个 sub_agent 一起
// 发出来，代码才有机会并发跑它们。这个"一起发"没有任何保证——它是模型
// 每一轮临场做的决定，这次做对了，下次可能就拆成三轮一个个来。这一章
// 新增一个 workflow 工具：模型把"分几个阶段、每个阶段派哪些子任务、
// 结果流向哪里"一次性写成一份计划交出来，之后的控制流由代码保证执行
...
func main() {
	...
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, `用法: ./ex21 "你的任务"  或  ./ex21 -c <session-id> "你的任务"`)
		os.Exit(1)
	}
	task := args[0]
