...
//
// 前十五章的知识全在代码里——工具怎么用、规矩是什么，写死在常量和
// system prompt 里，改一条都要重新编译。这一章加一种新知识：写在磁盘
// 上、模型按需读的说明书。第一步只解决"发现 + 注入"：让模型知道有哪些
// 说明书能读，以及怎么去读到一份的正文。
package main
...
func main() {
	...
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, `用法: ./ex16 "你的任务"  或  ./ex16 -c <session-id> "你的任务"`)
		os.Exit(1)
	}
	task := args[0]
