# Learn Agent the Hard Way — 练习 26：loop——谁来触发下一轮
#
# 前面二十五章，每一轮都是你开的口。这一章给模型一把闹钟：
# schedule_wakeup 让它自己说"多久之后再叫我，叫醒我的时候对我说这句
# 话"。到点了，进程自己开一轮，你不用在场。
#
# 这一章能成立，全靠前两章铺好的两样东西——练习 25 建的 EVENTS 事件
# 总线让"到点"这件事有地方汇入，练习 25 建的收件箱让到点的那一拍能塞进
# 正在跑的轮次。Go 版闹钟走 ctx 递给工具（练习 24 铺的路）；这一版
# Python 的工具从来没有 ctx 参数，沿用练习 25 CURRENT_CANCEL_EVENT 的
# 做法，加一个模块级全局 CURRENT_WAKER，在 run_interruptible 开始跑
# 一轮之前挂上、跑完摘下，schedule_wakeup 工具读它。
#
# 闹钟本身用 threading.Timer 实现，语义照抄 Go 版 waker：一个会话最多
# 一个待命的唤醒（再安排是替换不是叠加）、开始时刻跨 tick 累计（防漏的
# 上限才算得数）、固定节奏丢一拍无所谓（下一拍会补）、一次性模式绝不能
# 丢（丢了等于把循环杀了）。到点之后走一个专门的转发线程把 waker 内部
# 那个容量为 1 的队列桥接进 EVENTS——这跟 start_input_reader() 是同一个
# 模式：一个专职线程把一种事件源收成一路，喂给同一个分发器，不用改
# run_interruptible 的形状。
#
# 竞态那笔旧账在练习 25 已经交代过：Python 用线程实现读键盘的单一入口，
# 没有检测器能证明它无竞态，只能靠可观察结果说话。这一章新增的 waker
# 状态（timer/start）也由同一把锁保护，跟练习 25 是同一条纪律，不再
# 重复交代。
import json
import os
import platform
