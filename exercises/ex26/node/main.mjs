...
// JavaScript 的工具从来没有 ctx 参数，沿用练习 25 CURRENT_ABORT_SIGNAL
// 的做法，加一个模块级全局 CURRENT_WAKER，在 runInterruptible 开始跑
// 一轮之前挂上、跑完摘下，scheduleWakeupTool 读它。
//
// 闹钟本身用 setTimeout 实现，语义照抄 Go 版 waker：一个会话最多一个
// 待命的唤醒（再安排是替换不是叠加）、开始时刻跨 tick 累计（防漏的上限
// 才算得数）、固定节奏丢一拍无所谓（下一拍会补）、一次性模式绝不能丢
// （丢了等于把循环杀了）。JavaScript 单线程、EVENTS 又是一个不会阻塞的
// 异步队列，天然没有 Go 那种"channel 满了就阻塞"的效果——这一版用一个
// pendingTick 标记 + 一组等待者 Promise 手写出等价的背压：repeat 模式
// 见 pendingTick 非空就直接丢；一次性模式 await 到 pendingTick 被消费
// （runInterruptible/waitIdle 处理完一条 tick 后调用 wake.consumed()）
// 才真正送出去，效果上是同一个"必须送达"，只是用 await 代替了 Go 的
// 阻塞 channel send。
