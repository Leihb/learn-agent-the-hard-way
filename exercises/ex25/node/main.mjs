...
//
// 上一章读键盘和 confirm() 都靠 readLineSync()——一个同步阻塞调用，独占
// 事件循环，谁在读谁就挡住所有人，包括另一个还没 resolve 的 fetch()。
// 这一章把读键盘换成 node:readline（异步、事件驱动），confirm() 跟着
// 从"自己去读一行"改道成"发个请求、等答案"——轮次跑着的时候键盘照样
// 有人听：直接打是插话，/q 开头是排队。
//
// 语言差异先说在最前面：Go 版用一个 goroutine + select 把标准输入的
// 读者收成一个，四路 channel（轮次结束、信号、键盘、要问人）在同一个
// select 里仲裁。JavaScript 没有 select，这一版用一个共享的 EVENTS 队列
// （生产者-消费者的最小实现：push 一个事件，或者 await 下一个事件）把
// 四类事件收拢成一路，主循环变成一个 await EVENTS.next() 的分发循环——
// 效果上是同一件事，只是把"一个 select 语句"换成了"一个异步队列 + 一层
// if/else"。
//
// 竞态那笔旧账在这一版反而没有真实发生过：Node 从始至终只有一个线程，
// 旧版本的 readLineSync() 是同步的，两次"并发"的 confirm() 调用永远
// 没有机会真的同时读同一个文件描述符——第一个调用把事件循环整个占住，
// 第二个只能等它读完、事件循环重新排到自己才有机会开始。这不是设计上
// 的优点：那段时间里其它异步任务（包括另一个 sub_agent 的 fetch()）
// 全部被这一次同步读卡住，只是恰好副作用地避免了 Go 版那种真实的数据
// 竞争。这一章把 confirm() 换成异步之后，两个确认请求终于能真正同时
// "挂起"而不卡住事件循环——但这样一来，"两个问题会不会同时糊在屏幕上"
// 就从"结构上不可能"变成了"需要一套排队机制来保证"，于是这一版也需要
// 和 Go/Python 一样的 pending/backlog 仲裁，"发生了什么"里有真机实测。
...
async function runInterruptible(base, apiKey, model, reg, sess, window, input, box) {
...
// nextIdleLine 是空闲时（没有轮次在跑）等下一句话的地方，读的是同一个
// EVENTS 队列——'ask'/'done' 这时候不该出现（没有轮次在跑就没有工具在
// 问权限），忽略以防万一。
async function nextIdleLine() {
  for (;;) {
    const ev = await EVENTS.next();
    if (ev.tag === "line") return ev.line;
    if (ev.tag === "eof") {
      INPUT_CLOSED = true;
      return null;
    }
...
async function repl(base, apiKey, model, reg, sess, window, firstTask) {
  ...
  console.error(`[常驻模式：一行一句话。/exit 或 Ctrl+D 退出；轮次跑起来之后还能接着打字——直接打是插话，` +
    `${QUEUE_PREFIX}开头是排队等这一轮结束；Ctrl+C 打断这一轮]`);
  for (;;) {
    let line = firstTask;
    firstTask = "";
    if (!line) {
      process.stderr.write("\n> ");
      const text = await nextIdleLine();
      if (text === null) {
    ...
    await runInterruptible(base, apiKey, model, reg, sess, window, line, box);
    await runFollowUps(base, apiKey, model, reg, sess, window, box);
  }
  rl.close();
  try {
    sess.save();
