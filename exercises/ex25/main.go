...
//
// 上一章的进程常驻了，但轮次一跑起来就聋了：键盘没人读，你打的字只能攒在
// 终端缓冲里等这一轮结束。这一章把读键盘收进一个独立的 goroutine，轮次
// 跑着的时候照样听得见——直接打是插话（下一次发请求前塞进这一轮），
// /q 开头是排队（等这一轮收工单独跑）。
//
// 顺带还清一笔旧账：练习 20 的并发扇出留下过一个坑，几个子 agent 同时要
// 你批准时，几个 confirm 一起读同一个 os.Stdin，提示交错着打、回答落到谁
// 头上全看调度。既然键盘只剩一个读者，问人这件事也就只剩一条路了。
package main
...
func repl(base, apiKey, model string, reg *registry, sess *session, window int, firstTask string) int {
	...
	box := &inbox{}
	fmt.Fprintln(os.Stderr, "[常驻模式：一行一句话。/exit 或 Ctrl+D 退出；轮次跑起来之后还能接着打字——直接打是插话，"+queuePrefix+"开头是排队等这一轮结束；Ctrl+C 打断这一轮]")
	for {
		line := firstTask
		firstTask = ""
		if line == "" {
		...
		runInterruptible(base, apiKey, model, reg, sess, window, line, lines, box)
		runFollowUps(base, apiKey, model, reg, sess, window, lines, box)
	}
...
func runInterruptible(base, apiKey, model string, reg *registry, sess *session, window int, input string, lines <-chan string, box *inbox) {
