# Learn Agent the Hard Way — 练习 25：steer——往正在跑的轮次里插一句话
#
# 上一章的进程常驻了，但轮次一跑起来就聋了：读键盘和 confirm() 都是
# 同步阻塞调用，谁在跑就只有谁能碰标准输入。这一章把读键盘收进一个
# 后台线程，用一个队列把它和主循环解耦——轮次跑着的时候照样听得见：
# 直接打是插话（下一次发请求前塞进这一轮），/q 开头是排队（等这一轮
# 收工单独跑）。confirm() 也从"自己去读一行"改道成"发个请求、等队列
# 给答案"，这样键盘只剩一个真正的读者。
#
# 语言差异先说在最前面：Go 版用一个 goroutine + select 把标准输入的读者
# 收成一个，四路 channel（轮次结束、信号、键盘、要问人）在同一个 select
# 里仲裁。Python 没有 select 意义上的多路 channel 复用，这一版用一个
# 共享的 EVENTS 队列把四类事件收拢成一路，主循环变成一个阻塞在
# EVENTS.get() 上的分发器——效果上是同一件事，只是把"一个 select 语句"
# 换成了"一个带标签的队列 + if/elif 分发"。
#
# 竞态那笔旧账也要单独说清楚：练习 20 的并发扇出在 Go 版里翻过车——
# 好几个 goroutine 同时读同一个 bufio.Reader，用 -race 能抓到真正的数据
# 竞争。这一版 Python 用的是操作系统线程（threading.Thread），旧版本
# （练习 9 到练习 24）的 confirm() 一直是"哪个线程调用，就哪个线程直接
# sys.stdin.readline()"——can_fan_out 打开的并发扇出会让两个线程真的同时
# 竞争同一个文件描述符，这是一个真实存在的问题，只是 Python 没有像
# ThreadSanitizer/-race 那样的工具能抓住它（这不是内存安全意义上的数据
# 竞争，两个线程谁抢到内核那次 read() 都是合法的系统调用，只是抢到的人
# 不是你以为的那个）。这一章同样把读键盘收成一个线程、其余线程一律改道
# 找它要答案——修复的方式和 Go 版是同一个道理，只是没有一份检测器报告
# 能当作事后诸葛的证据，"发生了什么"里如实交代这一点。
#
# 取消依然是协作式的（上一章定下的取舍）：confirm() 现在需要在"等答案"
# 和"轮次被打断"之间做仲裁，但 registry.execute()/tool.execute() 的签名
# 从练习 9 起就没有携带取消令牌——这一章不给全部工具签名添一个参数，
# 而是用一个模块级全局 CURRENT_CANCEL_EVENT，在 run_interruptible 开始
# 跑一轮之前挂上、跑完摘下，confirm() 用短间隔轮询它来决定要不要提前
# 放弃等待。这是这一版故意做的简化，换来的是不用把一个参数改穿全部
# 已有的工具类；代价是响应打断有一个轮询间隔（本书取 50 毫秒），不是
# Go 那种真正的即时唤醒。
import json
import os
import platform
...
def run_interruptible(base, api_key, model, reg, sess, window, input_text, box):
    global CURRENT_CANCEL_EVENT, INPUT_CLOSED
...
# next_idle_line 是空闲时（没有轮次在跑）等下一句话的地方，读的是同一个
# EVENTS 队列——'ask'/'done' 这时候不该出现（没有轮次在跑就没有工具在
# 问权限），忽略以防万一。
def next_idle_line():
    global INPUT_CLOSED
    while True:
...
# 输入线程管，它只从 EVENTS 里接。
#
# 这不是一个工具，是运行环境的形态变了。往后几章要加的能力——定时唤醒、
# 后台任务跑完了来报信——全都得先有一个"还醒着的进程"才谈得上。
def repl(base, api_key, model, reg, sess, window, first_task):
    start_input_reader()
    box = Inbox()
    print(f"[常驻模式：一行一句话。/exit 或 Ctrl+D 退出；轮次跑起来之后还能接着打字——直接打是插话，"
          f"{QUEUE_PREFIX}开头是排队等这一轮结束；Ctrl+C 打断这一轮]", file=sys.stderr)
    while True:
        line = first_task
        first_task = ""
        if not line:
            ...
            if text is None:  # Ctrl+D
                print(file=sys.stderr)  # 补个换行，别让提示符黏在下一行
                break
            line = text
        ...
        if line in ("/exit", "/quit"):
            break
        run_interruptible(base, api_key, model, reg, sess, window, line, box)
        run_follow_ups(base, api_key, model, reg, sess, window, box)
    try:
        sess.save()
    except OSError as e:
        print(f"警告: 会话保存失败: {e}", file=sys.stderr)
