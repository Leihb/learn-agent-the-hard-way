...
#
# 练习 11 的会话持久化，续的是同一场对话——resume 的是同一个 session ID。
# 这一章要的是另一件事：完全重新开始的下一场对话，能不能记得上一场里
# 发生过的事。而且不是"记得越多越好"——记住之后，还要能改、能删。
import json
import os
import secrets
import subprocess
...
# compose_system_prompt 把 BASE_PROMPT、项目规则、记忆拼成一份 system
# prompt，蒸馏自 octo Compose 的分层方式：每层之间用同一个分隔符隔开，
# 某一层没有内容就跳过那一层。这份拼好的文字，从会话创建那一刻起冻结——
...
# force_rewrite 是这一章新加的：压缩会把 history 前半段整个换成一条摘要，
