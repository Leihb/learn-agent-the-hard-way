// Learn Agent the Hard Way — 练习 13：压缩——不是丢消息，是让模型总结它自己
//
// 练习 12 只喊话，不动手。这一章真正腾地方——但腾地方不等于扔掉：
// 把撑爆预算的那一段旧对话，让模型自己总结成一句话，原样保留最近的部分。
package main
...
func main() {
	...
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, `用法: ./ex13 "你的任务"  或  ./ex13 -c <session-id> "你的任务"`)
		os.Exit(1)
	}
	task := args[0]
