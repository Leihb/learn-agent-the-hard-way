...
# 派生三个 sub_agent，现在的代码是一个个 execute——每个 sub_agent 自己
# 就是一整场多轮的 LLM 对话，三个排队跑，总共要等的时间是三份时间加
# 起来。这一章把"这一批调用能不能并发跑"这个判断写成代码，用一个有
# 容量上限的信号量防止扇出多大都不设限，把本机资源或 provider 的并发
# 配额一次性打满。
import json
import os
import secrets
