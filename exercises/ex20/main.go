// Learn Agent the Hard Way — 练习 20：并行扇出与上限
//
// 上一章的 sub_agent 一次只能处理一个子任务。模型如果在同一轮里连续
// 派生三个 sub_agent，现在的代码是 for 循环里一个个 execute——每个
// sub_agent 自己就是一整场多轮的 LLM 对话，三个排队跑，总共要等的
// 时间是三份时间加起来。这一章把"能并发就并发"这个判断做成代码，同时用一个
// 有容量上限的 channel 当信号量，防止一次扇出把本地并发和 provider
// 连接一次性打满。
package main
