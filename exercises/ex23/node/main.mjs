// Learn Agent the Hard Way — 练习 23：沙箱——把 bash 关进笼子
//
// 练习 9 的权限系统拦的是命令字符串：deny 规则匹配不上的命令、被人批准
// 的命令，都会原样跑在你的机器上，能读你的 ~/.ssh、能写你的任何文件、
// 能连任何网络。这一章加一道 OS 强制的边界：-sandbox 开启后，bash 里
// 跑的每一条命令——不管权限系统放没放行、人批没批准——都只能写工作目录
// 和临时目录、读不了家目录下的密钥、连不了网。字符串匹配管的是"这条
// 命令看起来像什么"，沙箱管的是"这条命令实际做成了什么"。
...
const task = args[0];
if (!task) {
  console.error('用法: node main.mjs [-sandbox] "你的任务"  或  node main.mjs [-sandbox] -c <session-id> "你的任务"');
  process.exit(1);
}
...
// MCP 服务器子进程的 stdio 管道是活跃句柄，只要它们还开着，Node 的
// 事件循环就不会自然退出——Go/Python 的进程模型不会遇到这个问题（子
// 进程死活不影响父进程退出），这里需要一次显式 exit 来对应"main() 返回
// 就结束"的行为，不依赖事件循环自己清空。
process.exit(0);
