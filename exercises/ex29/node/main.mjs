// Learn Agent the Hard Way — 练习 29：更多工具——收网
//
// 这一章一口气加四个工具：grep、glob、web_search、web_fetch。没有任何
// 新机制——每一个都是"一份声明 + 一个干活的函数"，注册进练习 6 建的
// 注册表，一行一个。grep/glob 外包给 ripgrep（rg）；web_search/web_fetch
// 是这个 harness 第一次伸手到机器外面，web_fetch 先过练习 23 沙箱的
// allowNetwork 开关——那个埋了三章的字段，今天终于接上电。
