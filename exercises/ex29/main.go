...
// 注册表，一行一个。一章能装下四个工具，这件事本身就是论点：这套模式
// 你已经用了二十多章，它熟到可以流水线作业了。
//
// 四个工具分两组。grep/glob 是壳，真正干活的是 ripgrep——聪明的工具
// 知道什么时候该把活儿外包。web_search/web_fetch 是这个 harness 第一次
// 伸手到机器外面：搜索是一条多后端的降级链（有 API key 用 API，没有就
// 退到免费抓取），抓网页则要先过练习 23 沙箱的网络开关——那个埋了三章
// 一直没人消费的 allowNetwork 字段，今天终于接上了电。
package main
