...
不会完全一致，这属于正常的运行时波动，跟语言之间是否有系统性差异
无关。
...
- **并发扇出一定会更快吗**：不一定。云端 provider（这一章测的
  ...
  两种语言各自跑出的结果并不一致（其中一次并发反而更慢），问题不在
  代码，而在于"并发扇出在代码层面永远成立，但能不能换来真实的加速，
  取决于运行这些请求的后端本身撑不撑得住并发"——这句话在这次真机
  测试上得到了印证。

## 加分练习

1. 把并发上限改成 1，重新跑实验一，确认耗时退化成跟串行版本差不多——
   这是拿实测验证"限流阀容量决定了并发度"这句话，不靠肉眼猜。
