...
{{#endtab }}
{{#tab name="Python" }}
新建目录 `ex03`，新建 `main.py`。
代码开始变长了，但没有新协议——`send` 函数就是练习 2 的代码原样搬进来，
只改了一处：流式打印的同时把完整回复攒下来返回（失败返回 `None`）。
新东西全在 `main` 里：
...
{{#endtab }}
{{#tab name="JavaScript" }}
新建目录 `ex03`，新建 `main.mjs`。
代码开始变长了，但没有新协议——`send` 函数就是练习 2 的代码原样搬进来，
只改了一处：流式打印的同时把完整回复攒下来返回（失败返回 `null`）。
读终端输入第一次用到 Node 的内置模块 `node:readline`——内置的，仍然零依赖。
新东西全在 import 之后的主流程里：
...
（JavaScript 里叫 `push`），就是全世界所有 AI 对话产品"记忆力"的全部实现。
...
用户刚说的话。而失败时那行往回弹的代码（Go 的 `history = history[:len(history)-1]`，
Python/JavaScript 的 `history.pop()`）看着多余，
...
- **想退出**：`exit`，或者 Ctrl+D——EOF 会结束读输入的循环
  （Go 的 `stdin.Scan()` 返回 false，Python 的 `input()` 抛 `EOFError`，
  JavaScript 的 readline 迭代自然结束）。
...
3. 把 system 换成"无论用户用什么语言提问，你都只用英文回答"，
   ...
   再把 system 从数组里删掉试试。
4. 打印每轮的历史条数（`len(history)` / `history.length`），配着 stderr 的输入 token 数聊十轮，
   感受增长曲线。然后想：照这个速度，多少轮撞上模型的上下文窗口上限？
   撞上了该扔谁、留谁？别急着答——练习 12 和 13 就是这道题。
