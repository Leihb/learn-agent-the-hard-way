...
{{#endtab }}
{{#tab name="Python" }}
新建目录 `ex01`，在里面新建 `main.py`——不用建虚拟环境，不用装任何包，标准库就够：
...
{{#endtab }}
{{#tab name="JavaScript" }}
新建目录 `ex01`，在里面新建 `main.mjs`——`.mjs` 后缀让 Node 直接把它当 ES module 跑，
不需要 `package.json`，不用装任何包：
...
走本机，用练习 0 装好的 Ollama（key 随便填，它不校验，但字段不能空——
你的代码里检查了它）：
...
{{#tab name="Python" }}
```bash
python3 main.py "用一句话说明什么是 harness"
```

Python 3.10 以上即可，全程只用标准库。
{{#endtab }}
{{#tab name="JavaScript" }}
```bash
node main.mjs "用一句话说明什么是 harness"
```

Node 18 起自带 `fetch`，不需要 `npm install` 任何东西。
{{#endtab }}
{{#endtabs }}
...
（顺便，DeepSeek 把 harness 解释成了"测试夹具"，本机的 qwen3:4b-instruct 说是"缚具"。
都错了，而且错得各不相同——你正在跟着写的这本书就是纠正它们的。
...
而且通常带着 4xx/5xx 状态码。两个形状怎么走同一套解析，三种语言的答案不一样：

{{#tabs global="lang" }}
{{#tab name="Go" }}
Go 的 `http` 不把 4xx/5xx 当错误，响应体照常读——把两个形状合在一个结构体里解析，
是 Go 处理这类协议最省事的写法。
{{#endtab }}
{{#tab name="Python" }}
`urllib` 会把 4xx/5xx 直接抛成 `HTTPError`——但这个异常对象本身就是响应，
`e.read()` 把错误 JSON 照样读出来，跟成功路径汇合到同一套解析。
{{#endtab }}
{{#tab name="JavaScript" }}
`fetch` 对 4xx/5xx 不抛错（它只对网络层故障抛错），错误 JSON 就在 body 里——
不用任何分支，两个形状自然走同一套解析。
{{#endtab }}
{{#endtabs }}
...
- **回答是空的，但 `finish_reason=length`**：你用了思考型模型（比如 Ollama 的 `qwen3:4b`
  ——注意不是我们用的 `qwen3:4b-instruct`）。思考也计入 max_tokens，
  1024 个预算全被它想掉了，一个字都没轮到说；思考内容躲在响应里一个叫 `reasoning`
  的字段里，我们的代码没解析它。你刚学的"判断说完了没有，看 finish_reason 别看文字"
  在这里第一次派上用场——文字是空的，但 `length` 告诉你它不是没话说，是被掐断了。
