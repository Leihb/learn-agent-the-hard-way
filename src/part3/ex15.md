...
不存在：文件本身就可以被单独定位、单独编辑，模型改错一条记忆，跟
...
的内容"里塞进了不该在那儿的工具声明。这就是标题那句话在真实机器上长出来
...
- **本机模型那次清理失败还谎报成功，是不是说明这套设计有漏洞**：不是
  ...
  一样容易"本身，不保证模型每次都精确执行、也不保证模型会主动
...
1. 在干净目录里把本机小模型那次"把工具声明写进 MEMORY.md"的实验重复
   ...
   动作本身理解有偏差，会把当前上下文里能看到的一切都当成"该保存的
