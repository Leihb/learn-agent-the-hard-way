...
命令和续 turn 的判断在主循环里碰它。这一点三语言完全一致：Go 版
`goalBox` 是进程级全局变量，不像上一章的 `waker` 那样走 ctx——这是 Go
原版自己的取舍（交互式 CLI 一个进程就一个会话，全局最省事；octo 只在
server 形态下才换成每轮塞进 ctx 的版本，因为那边一个进程要同时伺候很多
会话），Python/JS 因此在这一点上没有跨语言分叉可交代：
...
const theGoal = new GoalBox(); // 单线程，不需要锁
```
{{#endtab }}
{{#endtabs }}
...
`run_interruptible` 里，SIGINT 的处理器原来只停闹钟，现在多一行：
...
`runInterruptible` 里，`onSigint` 原来只停闹钟，现在多一行：
...
Python 和 JavaScript 版跑同一个目标，真机结果逐字对应——都是一轮之内
`create_goal` → `write_file` → `read_file` → `update_goal(complete)`
四次调用收尾，日志文案和 Go 版完全一致：

```
[round 1] create_goal({"objective": "在 primes.txt 里写入前五个质数……"})
[goal 已立：……（已用 0 tokens）。/goal pause 暂停，/goal clear 删除]
[round 2] write_file({"path": "primes.txt", "content": "2\n3\n5\n7\n11\n"})
[round 3] read_file({"path": "primes.txt"})
[round 4] update_goal({"status": "complete"})
[goal → complete：……（已用 294 tokens）]
```
...
Python 版真机复现了实验二、三的核心机制：给一个"数数，不设预算"的
目标，deepseek-v4-flash 自己续了两轮（1→2→3），中途在轮次跑着的时候
发 `/goal pause`：

```
19.93  [round 2] bash({"command": "echo 3 >> count.txt && tail -n 3 count.txt"})
20.01  ===== 发送 /goal pause =====
20.01  [goal 已暂停：……（已用 2669 tokens）。/goal resume 恢复]
21.62  [round 3] schedule_wakeup({"delay_seconds": 60, "repeat": true, ...})
22.25  ✅ 本轮完成：count.txt 追加了 `3`……
22.25  [goal paused：……（已用 2669 tokens）]
```

`/goal pause` 立刻生效（打印"已暂停"），但**正在跑的这一轮没有被打断
——它继续跑完，把 3 追加完，goal 的续 turn 之后才不再触发**，跟 Go 版
"暂停改的是状态，续 turn 的判断只在轮次结束后发生"分毫不差。JavaScript
版机制相同（同一份 `handleGoalCommand`/`continuation` 逻辑），未单独
复测这一条真机场景。
...
Python 和 JavaScript 版都真机复现了预算越线：给一个 150 token 预算的
小目标（写 note.txt 并核对），两种语言都在第三次请求前越线——

```
 3.99  [goal 预算用完，已标成 budget_limited；收尾提示进了收件箱]
 3.99  [round 3] read_file({"path": "note.txt"})
 3.99  [插话进入这一轮：1 条，模型这就看到]
 6.19  [round 4] update_goal({"status": "complete"})
 6.19  [goal → complete：……（已用 507/150 tokens）]
```

真机还撞出一个和 Ollama 那次"自作主张塞预算"对称的怪癖：deepseek-v4-flash
在两种语言下都无视了收尾提示"除非目标真的已经完成，否则不要调
update_goal"，越线之后仍然自己调了 `update_goal(complete)`，
把状态从 `budget_limited` 掰成了 `complete`——这是被允许的（`setStatus`
只锁了"complete 回不去 active"这一条不变量，没有锁"budget_limited
不能变 complete"），模型这么做本身没有违反状态机的硬约束，只是没完全
听收尾提示的话。两种语言现象逐字一致，不是移植引入的差异。
...
实验五这两条路径没有在 py/js 上单独真机复测（跑到四分半、六万 token
或者反复试路径都太耗时间）——但它俩用的是实验一到四已经验证过的同一套
状态机（`update_goal` 的 complete/blocked 分支、`continuation` 的续
turn 判断），加上前面提到的 8 个单元测试（三语言各一份：
`goal_test.go` / `test_goal.py` / `test_goal.mjs`）逐条覆盖了零进度
刹车、越线记账、`complete` 不可诈尸这些不变量，机制上没有理由在这两条
路径上表现不同。
