...
// startInputReader 把"读键盘"收进一个 goroutine，一行一行往外送。
...
# start_input_reader 把"读键盘"收进一个线程，一行一行往 EVENTS
...
第三步，也是这一章最要紧的一行：**在哪儿把收件箱倒进历史**。加在
...
第五步，`runInterruptible` 从"守着两个 channel"长成真正的事件循环：
...
最后，一轮跑完，收件箱里可能还没清空：排队的那些，以及赶在收工那一瞬间
才进来、没赶上被取用的插话。
...
拿上一章的代码跑同一个场景，两个问题当场一起糊在屏幕上。更要命的是用竞态
...
注释直接点破了这一点：放在发请求之前单独倒进历史，是为了让模型看到的中途
...
留到最后，你眼睁睁看着它把你已经不要的活做完。octo 用一个
...
Part 7 的前两章都在搭骨架，从下一章起，新加的又变回工具了。
...
让正在跑的命令立刻停，那是 Ctrl+C 的活。这两者故意分开：插话是"改方向"，
打断是"别做了"。
...
1. **让插话能撤回。** 打完回车又后悔，是很常见的事。给收件箱加一个
   "把最后一条还没被取用的插话撤掉"的操作，`/undo` 触发。想清楚：
