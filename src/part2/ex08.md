...
{{#tabs global="lang" }}
{{#tab name="Go" }}
新增一个常量、一处 history 初始化的改动，外加给 usage 结构体加一个字段
（后面会用上）。完整文件在 `exercises/ex08/`。
...
{{#endtab }}
{{#tab name="Python" }}
新增一个常量、一处 history 初始化的改动。完整文件在
`exercises/ex08/python/main.py`。**语言红利**：Go 要先在 `response.Usage`
结构体里声明 `PromptTokensDetails` 字段才能读到缓存数，Python 的 JSON
本来就是动态 dict，不用预先声明任何字段——直接按路径取就行，取不到就给
默认值 0。
...
从响应里把"这次输入命中了多少缓存"读出来，取不到就是 0（先敲上，
后面解释为什么要看这个数）：
...
{{#tab name="JavaScript" }}
新增一个常量、一处 history 初始化的改动。完整文件在
`exercises/ex08/node/main.mjs`。**语言红利**：跟 Python 一样，JavaScript
的 JSON.parse 结果本来就是普通对象，不用预先声明字段——用可选链
`?.` 一路取到底，取不到就是 `undefined`，`?? 0` 兜个默认值。
